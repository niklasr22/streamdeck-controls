...
class Memory(SDUserApp):
    def __init__(
        ...
        self._icon = Image.open(icon_path).rotate(180)

        self._backside = Image.open(backside_path).rotate(180)
        self._memory_cards: list[Image.Image] = []
        for file in glob.glob(memory_pics_dir):
            self._memory_cards.append(Image.open(file).resize((72, 72)).rotate(180))
